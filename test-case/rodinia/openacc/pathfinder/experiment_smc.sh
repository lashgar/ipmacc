...
function run_benchmarks_nvprof()
{
 ...
 #do
  ...
  if [ "$DEBUG" == "1" ] ; then
   ...
   nvprof --print-gpu-trace ./pathfinder_cuda $args &> ../../$i.$3.txt
  else
   kernel=
   memory=
   launch=
   for (( j=0; j<$iter; j++)); do
    nvprof --print-gpu-trace ./pathfinder_cuda $args &> .log.txt
    #bash $path/$i/runnvprof &> .log.txt
    nvprof --print-api-summary ./pathfinder_cuda $args &> .log_api.txt
...
if [ "$RUN" == "1" ]; then
 ...
 run_benchmarks_nvprof $ROOT/openacc/ $ITER openacc100K pathfinder 100000 1000
 run_benchmarks_nvprof $ROOT/openacc/ $ITER openacc50K  pathfinder  50000 1000
fi
