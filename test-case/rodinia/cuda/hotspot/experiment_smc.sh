...
function run_benchmarks_nvprof()
{
 ...
 #do
  ...
  if [ "$DEBUG" == "1" ] ; then
   ...
   nvprof --print-gpu-trace ./hotspot 1024 $args  ../../data/hotspot/temp_1024 ../../data/hotspot/power_1024 output.out &> ../../$i.$3.txt
  else
   kernel=
   memory=
   launch=
   for (( j=0; j<$iter; j++)); do
    ...
    nvprof --print-gpu-trace ./hotspot 1024 $args  ../../data/hotspot/temp_1024 ../../data/hotspot/power_1024 output.out &> .log.txt 
    #bash $path/$i/runnvprof &> .log.txt
    #nvprof --print-api-summary ./pathfinder_cache $args &> .log_api.txt
    nvprof --print-api-summary ./hotspot 1024 $args  ../../data/hotspot/temp_1024 ../../data/hotspot/power_1024 output.out &> .log_api.txt 
...
if [ "$RUN" == "1" ]; then
 ...
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc1_1  hotspot 1 1
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc1_2  hotspot 1 2
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc2_4  hotspot 2 4
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc2_8  hotspot 2 8
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc4_8  hotspot 4 8
 run_benchmarks_nvprof $ROOT/cuda/ $ITER smc4_16 hotspot 4 16
fi
