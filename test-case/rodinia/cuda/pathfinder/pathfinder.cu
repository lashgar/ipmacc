        ...
        int border)
{
    ...
    for (int i=0; i<iteration ; i++){ 
                ...
                isValid)
        {
            ...
            // result[tx] = shortest + gpuWall[index]; // REP1
            __syncthreads(); prev[tx]=shortest + gpuWall[index]; __syncthreads(); // REP1
    ...
    if (computed){
        gpuResults[xidx]=prev[tx];		
        //gpuResults[xidx]=result[tx];		// REP
    }
}
