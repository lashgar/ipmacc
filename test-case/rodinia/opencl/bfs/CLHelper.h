...
//// HIHA
	...
	deviceListSize = ret_num_devices;
	//device_id_inused=device_id;
    	oclHandles.devices = (cl_device_id*)malloc(sizeof(cl_device_id)*1);
	oclHandles.devices[0]=device_id;
    	oclHandles.context = clCreateContext( NULL, 1, &device_id, NULL, NULL, &result);
        if(result!=CL_SUCCESS){
	...
	result = clGetContextInfo( oclHandles.context, CL_CONTEXT_DEVICES, 0, NULL, &size );
	cl_uint num_devices = (int) (size / sizeof(cl_device_id));
	...
	result = clGetContextInfo( oclHandles.context, CL_CONTEXT_DEVICES, size, device_list, NULL );
	if( result != CL_SUCCESS ) { printf("ERROR: clGetContextInfo() failed\n"); exit(-1); }

	// create command queue for the first device
        oclHandles.queue = clCreateCommandQueue( oclHandles.context, device_list[0], 0, NULL );
	if( !oclHandles.queue ) { printf("ERROR: clCreateCommandQueue() failed\n"); exit(-1); }
  ...
  // }
   ...
   // //--cambine-5: Load CL file, build CL program object, create CL kernel object
    ...
    if ((resultCL != CL_SUCCESS) || (oclHandles.program == NULL))
    {
                ...
                char *build_log=NULL;
		cl_uint __ipmacc_clerr;
                __ipmacc_clerr=clGetProgramBuildInfo(oclHandles.program, oclHandles.devices[0], CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
                if(__ipmacc_clerr!=CL_SUCCESS){
                        printf("OpenCL Runtime Error in clGetProgramBuildInfo! id: %d\n",__ipmacc_clerr);
                }
                build_log = (char*)malloc((log_size+1));
                // Second call to get the log
                __ipmacc_clerr=clGetProgramBuildInfo(oclHandles.program, oclHandles.devices[0], CL_PROGRAM_BUILD_LOG, log_size, build_log, NULL);
                if(__ipmacc_clerr!=CL_SUCCESS){
                        printf("OpenCL Runtime Error in clGetProgramBuildInfo! id: %d\n",__ipmacc_clerr);
                }
