...
class codegen(object):
    ...
    def codegen_memCpy(self, des, src, size, type):
        if self.target_platform=='CUDA':
            ...
            print 'error: unimplemented platform'
            exit(-1)
    def codegen_devPtrDeclare(self, type, name):
        if self.target_platform=='CUDA':
            return self.devPtrDeclare_cuda(type, name)
        elif self.target_platform=='OPENCL':
            return self.devPtrDeclare_opencl(type, name)
        else:
            print 'error: unimplemented platform'
            exit(-1)
    ...
    def memCpy_cuda(self, des, src, size, type):
        ...
        return codeC
    def devPtrDeclare_cuda(self, type, name):
        return type+' '+name+';\n'
    ...
    def constructKernel_opencl(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims):
        ...
        code=code+'(__global '+(', __global '.join(args))+')'
        proto=code+';\n'
        code+='{\n'
    ...
    def memCpy_opencl(self, des, src, size, type):
        ...
        return codeC
    def devPtrDeclare_opencl(self, type, name):
        return 'cl_mem '+name+';\n'
        ...
        for i in range(0,len(self.oacc_copys)):
            ...
            for j in self.oacc_copys[i].split('\n'):
                ...
                if varmapper_allocated_found==False:
                    ...
                    vardeclare+=self.codegen_devPtrDeclare(type,dname)
                ...
                if incom=='true':
                    ...
                    codeC+=self.codegen_memCpy(dname, varname, size, 'in')
                elif incom=='false':
                    codeC+='ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                    codeC+=self.codegen_memCpy(varname, dname, size, 'out')
                else:
