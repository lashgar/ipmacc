...
class scanner(object):
    def __init__(self, fname=None, foname=None):
        ...
        self.fohandle.write('<scanner version=\'0.1.7b\'>\n') #initial the output XML file
    ...
    def labeling(self):
        while self.lastchar:
            ...
            if DEBUG==2:
                ...
                #exit (-1)
            if self.lastchar=='/' and self.bcomment==False and self.lcomment==False:
                #potentially comment
                self.temp_read()
                #self.temp_read()
                if self.lastchar=='/':
                ...
                if self.lcomment==True:
                    ...
                    self.pragma=False
                self.read()
            elif self.lcomment==False and self.lastchar=='*' and self.bcomment==True:
