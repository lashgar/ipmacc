...
PROFILER=False
SMCCONSERV=True   # enable this to implement cache directive with least optimization (handle design corners)
...
class codegen(object):
    ...
    def __init__(self, target, foname, nvcc_args, optimizations, perforationconf):
        ...
        self.perforationfixtyp = 'non' if perforationconf=='' else perforationconf
...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def oacc_smc_squeez_size(self, ctadimx, up, dw, flag):
        if flag:
            if   ctadimx==str(eval(up+'+1')):
                length  =ctadimx+('' if dw=='0' else '+'+dw)
            elif ctadimx==str(eval(dw+'+1')):
                length  =ctadimx+('' if up=='0' else '+'+up)
            else:
                length  =ctadimx+('' if dw=='0' else '+'+dw)+('' if up=='0' else '+'+up)
        else:
            length  =ctadimx+('' if dw=='0' else '+'+dw)+('' if up=='0' else '+'+up)
        return length
        ...
        if len(smcinfo)>0:
            ...
            for [v, t, st, p, dw, up, div, a, dimlo, dimhi, w_dw, w_up, dim2low, dim2high, pivot2, dw2range, up2range, w_dw2range, w_up2range, vcode] in smcinfo:
                ...
                if dim2high=='' and dim2low=='':
                    ...
                    length   = self.oacc_smc_squeez_size(ctadimy, up, dw, f_is_stataic_p)
                    length_2 = self.oacc_smc_squeez_size(ctadimx, up2range, dw2range, f_is_stataic_p2)
                    # declare local memories
                    decl+='\n/* declare the shared memory of '+v+' */\n'
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE' or st=='FETCH_CHANNEL':
                        ...
                        if isfusion:
                            ...
                            if SMCCONSERV:
                                ...
                                if not f_is_stataic_p:
                                    ...
                                    datafetch+=self.prefix_kernel_smc_startpointer+v+'='+p+'-'+dw+';\n'
                                    #datafetch+=self.prefix_kernel_smc_endpointer+v+'='+'(lastrow)?'+dimhi+'-1:'+self.prefix_kernel_smc_startpointer+v+'+'+up+'+'+dw+';\n'
                                    datafetch+=self.prefix_kernel_smc_endpointer+v+'='+self.prefix_kernel_smc_startpointer+v+'+'+up+'+'+dw+';\n'
                                if not f_is_stataic_p2:
                                    ...
                                    datafetch+=self.prefix_kernel_smc_startpointer+v+'_2d='+pivot2+'-'+dw2range+';\n'
                                    # datafetch+=self.prefix_kernel_smc_endpointer+v+'_2d='+'(lastcol)?'+dim2high+'-1:'+self.prefix_kernel_smc_startpointer+v+'_2d+'+up2range+'+'+dw2range+';\n'
                            ...
                            #             __kernel_smc_startpointer_"""+v+""",
                            #             __kernel_smc_endpointer_"""+v+""",
                            #             __kernel_smc_startpointer_"""+v+"""_2d,
                            #             __kernel_smc_endpointer_"""+v+"""_2d);
                            # }
                            #        """
                        ...
                        if isfusion:
                            ...
                            codeblock_8=''
                            if tagbasedcache_size!='':
                                codeblock_8+='unsigned int sidx = (idx*'+dim2high+'+idx2)&('+tagbasedcache_size+'-1);\n'
                                codeblock_8+=self.prefix_kernel_smc_varpref+v+'[sidx]='+v+'[idx*'+dim2high+'+idx2];\n'
                                codeblock_8+=self.prefix_kernel_smc_tagpref+v+'[sidx]=(idx*'+dim2high+'+idx2);\n'
                            else:
    ...
    def smc_kernelBody_parse(self, kernelB, changeRangeIdx_start, changeRangeIdx_end, arg_smc_uid, arg_smc_subarray_name, arg_smc_type, arg_smc_subarray_d2_size, arg_smc_range_p, arg_smc_range_dw, arg_smc_range_up):
        ...
        for arrayRef in re.finditer('\\b'+v+'[\\ \\t\\n\\r]*\[',kernelB[changeRangeIdx_start:changeRangeIdx_end]):
            #skip to the end of variable
            [iswrite, assignmentIdx, assignmentOpr, writeIdx_str, writeIdx_end, writeIdx_loc, writeIdx_val] = self.util_is_written_to(kernelB, arraccs[idx], changeRangeIdx_start+arrayRef.start(0))
            # arrayRef_it=changeRangeIdx_start+arrayRef.start(0)
            # arrayRef_pcnt=0
            # done=False
            # while not done:
            ...
            if kid==kernelId:
                for vop in vlist.split(','):
                    ...
                    if vop.count(':')==1:
                        ...
                        diverge = 'false' # default to RBI, set true for RBC
            ...
            if root.tag=='pragma':
                ...
                if (self.target_platform=='CUDA' or self.target_platform=='OPENCL') and root.attrib['independent']=='true': #independent:
                    ...
                    if root.attrib.get('incoperator')=='+' or root.attrib.get('incoperator')=='-':
                        if not prfr_en:
                            ex_incstep = root.attrib.get('incstep')
                        elif prfr_ty=='stride':
                            ex_incstep = '('+prfr_prm[0]+'*'+root.attrib.get('incstep')+')'
                        elif prfr_ty=='rate':
                            ex_incstep = '((1/(1-'+prfr_prm[0]+'))*'+root.attrib.get('incstep')+')'
                        else:
                            print 'unknown perforation type.'
                            print 'aborting()'
                            exit(-1)
                        if prfr_en and prfr_fix!='non':
                            iteratorVal_next=('('+root.attrib.get('init')+')'+root.attrib.get('incoperator'))+ex_incstep+'*('+dimension_iterator+'+1);'
                        iteratorVal=('('+root.attrib.get('init')+')'+root.attrib.get('incoperator'))+ex_incstep+'*('+dimension_iterator+');'
                        ...
                        if prfr_en and prfr_fix!='non':
                            ...
                            iteratorVal_next=('('+root.attrib.get('init')+')'+root.attrib.get('incoperator'))+powerfunction+';'
                    else:
                    ...
                    if prfr_en and prfr_fix!='non':
                        code+='int __ipmacc_internal_'+root.attrib.get('iterator')+'_next ='+iteratorVal_next+'\n'
                    ...
                    if prfr_en:
                        ...
                        if DEBUGPRFR:
                            ...
                            for [arraccs, indecis, dependt, arraccs_loc] in prfr_kernel_reads:
                                ...
                                print '========='
                        if prfr_fix!='non':
                            prfr_fix_carry_condition = '(__ipmacc_internal_'+root.attrib.get('iterator')+'_plus_1)!=__ipmacc_internal_'+root.attrib.get('iterator')+'_next'
                            codeinside+='// perforation fixup\n'
                            codeinside+='int __ipmacc_internal_'+root.attrib.get('iterator')+'_plus_1 = '+root.attrib.get('iterator')+'+1;\n'
                            codeinside+='if('+prfr_fix_carry_condition+')\n{\n'
                            codeinside+='int '+root.attrib.get('iterator')+'='+'__ipmacc_internal_'+root.attrib.get('iterator')+'_plus_1;\n'
