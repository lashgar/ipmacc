        ...
        if len(smcinfo)>0:
            ...
            for [v, t, st, p, dw, up, div, a, dimlo, dimhi, w_dw, w_up, dim2low, dim2high, pivot2, dw2range, up2range, w_dw2range, w_up2range] in smcinfo:
                ...
                if dim2high=='' and dim2low=='':
                    ...
                    smc_select_calls+='__forceinline__ __device__ '+t.replace('*','')+' __smc_select_'+str(a)+'_'+v+'(int index, int down, int up, '+t+' g_array, '+t+' s_array, int vector_size, int pivot, int before, unsigned int startptr, int lbnd, int ubnd){\n'
                    if div=='false':
                    ...
                    decl+='__shared__ unsigned int '+self.prefix_kernel_smc_startpointer+v+';\n'
                    decl+='__shared__ unsigned int '+self.prefix_kernel_smc_endpointer+v+';\n'
                    decl+='__shared__ unsigned int '+self.prefix_kernel_smc_startpointer+v+'_2d;\n'
                    decl+='__shared__ unsigned int '+self.prefix_kernel_smc_endpointer+v+'_2d;\n'
                    decl+=self.prefix_kernel_smc_endpointer+v+'=-1;\n'
                    decl+=self.prefix_kernel_smc_startpointer+v+'=-1;\n'
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE' or st=='FETCH_CHANNEL':
                        ...
                        datafetch+='  for(kk2=threadIdx.y; kk2<__ipmacc_length_2d; kk2+=blockDim.y)\n'
                        datafetch+='  {\n'
                        datafetch+='   int idx2='+self.prefix_kernel_smc_startpointer+v+'_2d+kk2;\n'
                        datafetch+='   if(idx2<('+dim2high+') && idx2>=('+dim2low+'))\n'
                        datafetch+='   {\n'
                        datafetch+='for(kk=threadIdx.x; kk<__ipmacc_length; kk+=blockDim.x)\n'
                        datafetch+='{\n'
                        datafetch+=' int idx='+self.prefix_kernel_smc_startpointer+v+'+kk;\n'
                        datafetch+=' if(idx<('+dimhi+') && idx>=('+dimlo+'))\n'
                        datafetch+=' {\n'
