            ...
            if i0.strip()=='sort':
                ...
                for variable in i3.split(','): # FIXME: this stuck on complex range specifiers, like (suba[0:foo(1,2,3)],subb[0:foo(3,4,5)])
                    ...
                    if True:
                        ...
                        #decl+='//__shared__ '+vtype_noptr+' '+vname+'_sort_local '+'['+_sort_global_size+'];\n'
                        decl+=vtype_noptr+' *'+vname+'_sort_local = ('+vtype_noptr+'*)&__ptr_dynamic_shared_allocation['+_sort_local_offset+'];\n'
                        #decl+=vtype_noptr+' *'+vname+'_sort_local = ('+vtype_noptr+'*)&__ptr_dynamic_shared_allocation[0];\n'
                        decl+='// initialize the '+vname+'_sort_local array'
                        decl+="""
                              {
                                 """+vname+"""_sort_local[threadIdx.x] = 1<<30;
                                 """+vname+"""_sort_local[threadIdx.x+blockDim.x] = 1<<30;
        ...
        try:
            if root.tag=='pragma':
                if root.attrib.get('directive')=='kernels':
                    ...
                    self.oacc_kernelsConfig_update(id, self.algo_get_preferred_width(id)) #self.algorithm_execution_width)
                    for ch in root:
