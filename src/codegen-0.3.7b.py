...
from wrapper import srcml_code2xml, srcml_get_fcn_calls, srcml_get_var_details, srcml_get_parent_fcn, srcml_get_all_ids, srcml_get_declared_vars, srcml_find_var_size, srcml_get_fwdecls
...
class codegen(object):
    def __init__(self, target, foname=None):
        ...
        self.intrinsic_calls= [ 'acos', 'acosh', 'acospi', 'asin', 'asinh', 'asinpi', 'atan', 'atan2', 'atanh', 'atanpi', 'atan2pi',
                                ...
                                'prefetch' ] # opencl calls which are available within kernel
...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def forward_declare_find(self):
        # find the declaration of the undeclared calls/types listed in self.active_calls nad self.active_types
        # the declaration will be stored in self.active_calls_decl nad self.active_types_decl for future uses
        ...
        [self.active_types_decl, self.active_calls_decl]=srcml_get_fwdecls(codein, undecls_types, undecls_calls, self.intrinsic_calls)
        # 5) debugging
        if DEBUGFWDCL:
    ...
    def oacc_clauseparser_data(self, clauses):
        ...
        for it in ['copy', 'pcopy', 'present_or_copy', 'copyout', 'pcopyout', 'present_or_copyout', 'copyin', 'pcopyin', 'present_or_copyin', 'create', 'present', 'present_or_create']:
            expressionAll=expressionAll+self.oacc_data_clauseparser(clauses,it,'false','false')
        return [expressionIn, expressionAlloc, expressionOut, expressionAll]

    def oacc_clauseparser_data_ispresent(self, clause):
        for it in ['pcopy', 'present_or_copy', 'pcopyout', 'present_or_copyout', 'pcopyin', 'present_or_copyin', 'present', 'present_or_create']:
            if clause==it:
                return True
        return False
    ...
    def codegen_constructKernel(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims):
        if self.target_platform=='CUDA':
            return self.constructKernel_cuda(args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, self.oacc_kernelsTemplates[kernelId])
        elif self.target_platform=='OPENCL':
            return self.constructKernel_opencl(args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, self.oacc_kernelsTemplates[kernelId])
        else:
            print 'error: unimplemented platform'
            exit(-1)
...
# API-based IMPLEMENTATION FIXME
        ...
        if self.target_platform=='CUDA':
            code=dvar+'=('+type+ast+')'+fcall+'((void*)'+amp+hvar+','+size+');\n'
        elif self.target_platform=='OPENCL':
            code=dvar+'=(cl_mem)'+fcall+'((void*)'+amp+hvar+','+size+');\n'
        else:
            print 'error: unimplemented platform'
            exit(-1)
        return code
    ...
    def codegen_accCopyin(self, host, dev, bytes, type, present, scalar_copy):
        ...
        code=''
        if self.target_platform=='CUDA':
            code+=dev+'=('+type+ast+')acc_'+present+'copyin((void*)'+host+','+bytes+');\n'
        elif self.target_platform=='OPENCL':
    ...
    def codegen_accPresent(self, host, dev, bytes, type):
        code=''
        if self.target_platform=='CUDA':
            code+=dev+'=('+type+')acc_present((void*)'+host+');\n'
        elif self.target_platform=='OPENCL':
            code+=dev+'=(cl_mem)acc_present((void*)'+host+');\n'
        else:
            print 'error: unimplemented platform'
            exit(-1)
        return code
    ...
    def appendKernelToCode_cuda(self, kerPro, kerDec, kerId, forDims, args):
        ...
        self.cuda_kerneldecl +=cleanKerDec #instead of kerDec
    ...
    def reduceVariable_cuda(self, var, type, op, ctasize):
        ...
        code+='}\n'
        return code
    def constructKernel_cuda(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, template):
        code='__global__ '+template+' void '+self.prefix_kernel_gen+str(kernelId)
        code=code+'('+(','.join(args))+')'
        proto=code+';\n'
        code+='{\n'
    ...
    def syncDevice_opencl(self):
        code=''
        code+='if (getenv("IPMACC_VERBOSE")) printf("IPMACC: Synchronizing the region with host\\n");\n'
        code+='clFlush(__ipmacc_command_queue);\n'
        return code
    ...
    def appendKernelToCode_opencl(self, kerPro, kerDec, kerId, forDims, args):
        ...
        cleanKerDec=cleanKerDec.replace('\n','\\n')
        kernelInvoc='\n/*kernel call statement*/\n'
        kernelInvoc+='const char* kernelSource'+kerId_str+' ="'+cleanKerDec+'";\n'
        ...
        char *build_log=NULL;
        __ipmacc_clerr=clGetProgramBuildInfo(__ipmacc_clpgm"""+kerId_str+""", NULL, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
        if(__ipmacc_clerr!=CL_SUCCESS){
            printf("OpenCL Runtime Error in clGetProgramBuildInfo! id: %d\\n",__ipmacc_clerr);
        }
        build_log = (char*)malloc((log_size+1));
        // Second call to get the log
        __ipmacc_clerr=clGetProgramBuildInfo(__ipmacc_clpgm"""+kerId_str+""", NULL, CL_PROGRAM_BUILD_LOG, log_size, build_log, NULL);
        if(__ipmacc_clerr!=CL_SUCCESS){
            printf("OpenCL Runtime Error in clGetProgramBuildInfo! id: %d\\n",__ipmacc_clerr);
        }
        ...
        for j in range(0,len(args)):
            ...
            if pointer:
                kernelInvoc+='__ipmacc_clerr=clSetKernelArg(__ipmacc_clkern'+kerId_str+', '+str(j)+', sizeof(cl_mem), (void *)&'+dname+');\n'
            else:
    ...
    def reduceVariable_opencl(self, var, type, op, ctasize):
        ...
        code+='}\n'
        return code
    def constructKernel_opencl(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, template):
        code ='__kernel '+template+' void '+self.prefix_kernel_gen+str(kernelId)
        suff_args=[]
    ...
    def code_descendentRetrieve(self, root, depth):
        ...
        if root.tag == 'pragma' and root.attrib.get('directive')=='kernels':
            ...
            expressionAll=''
            copyoutId=-1
            [expressionIn, expressionAlloc, expressionOut, expressionAll] = self.oacc_clauseparser_data(str(root.attrib.get('clause')))
            if DEBUGVAR:
...
# TO HERE
    ...
    def preprocess_by_gnu_cpp(self, codein):
        ...
        p2 = Popen(["cpp", "-E", "-I"+os.path.dirname(os.path.realpath(__file__))+"/../include/", "-I"+os.path.dirname(self.foname)+"/./"], stdin=p1.stdout, stdout=PIPE)
    ...
    def var_copy_genCode(self):
        ...
        for i in range(0,len(self.oacc_copys)):
            ...
            for j in cp_expression.split('\n'):
                ...
                if clause=='present':
                    codeCin+='ipmacc_prompt("IPMACC: memory getting device pointer for '+varname+'\\n");\n'
...
#                    else:
                ...
                if REDUCTION_TWOLEVELTREE:
                    ...
                    codeM+= type+' '+self.prefix_kernel_reduction_array+varname+'=NULL;\n'
...
#                if incom=='true':
                if reduc=='':
                    ...
                    if REDUCTION_TWOLEVELTREE:
                        ...
                        iterator =self.prefix_kernel_reduction_iterator
                        codeCout+='\n/* secon-level reduction on '+varname+' */\n'
                        codeCout+='{\n'
                        codeCout+='int '+iterator+'=0;\n'
                        codeCout+='for('+iterator+'='+griddimen+'-1; '+ iterator+'>0; '+iterator+'-=1){\n'
...
#        self.astRoot = tree.getroot()
    ...
    def oacc_clauseparser_loop_isindependent(self,clause):
        ...
        return [indep, private, reduction, gang, vector]
...
#            print 'checking variable > \''+vars[i]+'\''
            ...
            if not priv:
                try :
                    ...
                    function_args.append(arg_type+arg_name+('__ipmacc_reductionarray_internal' if redu else ''))
...
#            code+='int '+i+';\n'
    ...
    def carry_loopAttr2For(self, root, independent, private, reduction, gangs, vectors):
        ...
        if root.tag=='pragma':
            if root.attrib.get('directive')=='kernels':
                for ch in root:
                    self.carry_loopAttr2For( ch, False, private, reduction, gangs, vectors)
            elif root.attrib.get('directive')=='loop':
                [indep, priv, reduc, gang, vector]=self.oacc_clauseparser_loop_isindependent(root.attrib.get('clause'))
                for ch in root:
                    self.carry_loopAttr2For(ch, indep, private+priv, reduction+reduc, gangs+gang, vectors+vector)
            else:
...
#            print 'it could be something like -> '+self.kernel_forSize_CReadable(self.find_kernel_forSize_Recursive(self.oacc_kernels[i]))
        ...
        k.var_copy_assignExpDetail(FDims, (self.blockDim_cuda if self.target_platform=='CUDA' else self.blockDim_opencl))
        k.var_copy_genCode()
        ##k.var_copy_showAll()
        ##k.varmapper_showAll()
...
if options.filename=="":
    ...
    print 'IPMACC: The target code is <'+options.target_platform+'> the output file is <'+options.filename+'>'
...
k = codegen(options.target_platform, options.filename)
