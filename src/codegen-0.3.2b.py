...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def util_copyIdAppend(self, expressionIn, expressionAlloc, expressionOut, depth, copyoutId, expressionAll):
        if expressionIn!='':
...
# HERE
            ...
            copyoutId=self.util_copyIdAppend(expressionIn, expressionAlloc, expressionOut, depth, copyoutId, expressionAll)
#            if expressionIn!='':
#                self.code=self.code+('\t'*depth)+self.prefix_dataalloc+str(self.oacc_copyId)+'();'
#                self.code=self.code+('\t'*depth)+self.prefix_datacp+str(self.oacc_copyId)+'();'
#                self.oacc_copys.append([self.oacc_kernelId,expressionIn])
                ...
                for fcall in func.findall(".//FuncCall/ID"):
                    if str(fcall.get('uid')).strip()==kn.strip():
                        ...
                        if funcName=="kernel":
                            ...
                            #self.oacc_kernelsParent.append(func.find('Decl').get('uid').replace(',','')) #FIXME
                            self.oacc_kernelsParent.append(fname) 
                        else:
                            #self.oacc_copysParent.append(fcall.get('uid'))
                            #self.oacc_copysParent.append(func.find('Decl').get('uid').replace(',','')) #FIXME
        ...
        for i in range(0,len(self.oacc_copys)):
            ...
            for j in cp_expression.split('\n'):
                ...
                if incom=='true':
                    ...
                    codeC+=self.codegen_accCopyin(('&' if scalar_copy else '')+varname, dname, size, type, ('p' if present=='true' else ''))
                elif incom=='false':
                    codeC+='ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                    #codeC+=self.codegen_memCpy(('&' if scalar_copy else '')+varname, dname, size, 'out')
                    #codeM+=self.codegen_memAlloc(dname,size,varname,type,scalar_copy)
                    codeC+=self.codegen_accPCopyout(('&' if scalar_copy else '')+varname, dname, size, type)
                else:
