...
# API-based IMPLEMENTATION FIXME
        ...
        if GENMULTIDIMTB:
            ...
            for f in range(0,len(forDims)):
                ...
                if   f==0:
                    ...
                    print 'Error! Multi-dimensional grid is limited to 3. Disable GENMULTIDIMTB in condegen.py'
                    exit(-1)
                dim3_gridblock+='__ipmacc_blockDim.'+ch+'='+self.blockDim_cuda_xyz+';\n'
                dim3_gridblock+='__ipmacc_gridDim.'+ch+'=(('+forDims[f]+')/__ipmacc_blockDim.'+ch+')+1;\n'
            if DEBUGMULTIDIMTB:
            ...
            for [v, t, st, p, dw, up, div, a, dimlo, dimhi, w_dw, w_up, dim2low, dim2high, pivot2, dw2range, up2range, w_dw2range, w_up2range] in smcinfo:
                ...
                if dim2high=='' and dim2low=='':
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE' or st=='FETCH_CHANNEL':
                        ...
                        datafetch+=self.prefix_kernel_smc_endpointer+v+'='+'(lastcol)?'+dimhi+'-1:blockDim.x+'+self.prefix_kernel_smc_startpointer+v+'-1;\n'
                    ...
                    smc_select_calls+='__forceinline__ __device__ '+t.replace('*','')+' __smc_select_'+str(a)+'_'+v+'(int index, int down, int up, '+t+' g_array, '+t+' s_array, int vector_size, int pivot, int before, int startptr, int lbnd, int ubnd){\n'
                    if div=='false':
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE' or st=='FETCH_CHANNEL':
                        ...
                        if isfusion:
                            ...
                            datafetch+=self.prefix_kernel_smc_endpointer+v+'='+'(lastrow)?'+dimhi+'-1:blockDim.y+'+self.prefix_kernel_smc_startpointer+v+'-1;\n'
                            datafetch+=self.prefix_kernel_smc_endpointer+v+'_2d='+'(lastcol)?'+dim2high+'-1:blockDim.x+'+self.prefix_kernel_smc_startpointer+v+'_2d-1;\n'
                        ...
                        kernelB=kernelB.replace(fcall,datafetch+'\n'+fcall)
                    # construct the smc_select_ per array for READ
                    smc_select_calls+='__forceinline__ __device__ '+t.replace('*','')+' __smc_select_'+str(a)+'_'+v+'(int index, '+t+' g_array, '+t.replace('*','')+' s_array['+length+']['+length_2+'], int startptr, int startptr2, int dim2size){\n'
                    if div=='false':
                        smc_select_calls+='// the pragmas are well-set. do not check the boundaries.\n'
                        smc_select_calls+="""    int idx=index/dim2size;\nint idx2=index%dim2size;\n"""
                        if GENDEBUGCODE:
    ...
    if(!((idx2-startptr2)<("""+length_2+"""))){\n
        REPLACECALL()\n
            assert((idx2-startptr2)<"""+length_2+""");\n
                        ...
                        smc_select_calls+='return s_array[idx-startptr][idx2-startptr2];\n'
                    else:
    ...
    if(!((idx2-startptr2)<20)){\n
        REPLACECALL()\n
            assert((idx2-startptr2)<16);\n
                    ...
                    if div=='false':
                        smc_write_calls+='// the pragmas are well-set. do not check the boundaries.\n'
                        smc_write_calls+='int idx=index/dim2size;\nint idx2=index%dim2size;\n'
                        smc_write_calls+='s_array[idx-startptr][idx2-startptr2]=value;\n'
                    else:
                        print 'Warning! divergent is not implemented for 2D SMC. Falling back to non-divergent'
                        smc_write_calls+='int idx=index/dim2size;\nint idx2=index%dim2size;\n'
                        smc_write_calls+='s_array[idx-startptr][idx2-startptr2]=value;\n'
                    ...
                    indexCalculationCode=self.smc_get_indexCalculation(readIdxSet, readIdxList, v, dim2high, writeIdxSet, writeIdxList, unifiedIdxSet, unifiedIdxList)
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE'  or st=='FETCH_CHANNEL':
                        kernelB=kernelB.replace(fcallen,'#undef '+v+'\n'+fcallen)
                    # Fusion Control
                    fusionSMC.append([p,dw,up,pivot2,dw2range,up2range])
                    # 
                    pfreelist.append(a)
            for ids in pfreelist:
    ...
    def decompose1Dindexto2D(self, index,dim1,dim2):
        try:
            [p1,p2]=index.split('+')
            if p1.find('*')!=-1:
                C=p1
    ...
    def smc_get_indexCalculation(self, readIdxSet, readIdxList, v, dim2high, writeIdxSet, writeIdxList, unifiedIdxSet, unifiedIdxList):
        ...
        for rd in range(0,len(unifiedIdxSet),1):
            ...
            [A,B] = self.decompose1Dindexto2D(unifiedIdxSet[rd], '0', dim2high)
            #indexCalculationCode+='\tint __ipmacc_smc_index_'+v+'_'+str(rd)+'_base='+unifiedIdxSet[rd]+';\n'
            indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim1 '+A+'-'+self.prefix_kernel_smc_startpointer+v+'\n'
            #indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim1 '+unifiedIdxSet[rd]+'-'+self.prefix_kernel_smc_startpointer+v+'\n'
            #indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim1 (__ipmacc_smc_index_'+v+'_'+str(rd)+'_base/'+dim2high+')-'+self.prefix_kernel_smc_startpointer+v+'\n'
            #indexCalculationCode+='\tint __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim1=(__ipmacc_smc_index_'+v+'_'+str(rd)+'_base/'+dim2high+')-'+self.prefix_kernel_smc_startpointer+v+';\n'
            indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim2 '+B+'-'+self.prefix_kernel_smc_startpointer+v+'_2d\n'
            #indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim2 '+unifiedIdxSet[rd]+'-'+self.prefix_kernel_smc_startpointer+v+'_2d\n'
            #indexCalculationCode+='\t#define __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim2 (__ipmacc_smc_index_'+v+'_'+str(rd)+'_base%'+dim2high+')-'+self.prefix_kernel_smc_startpointer+v+'_2d\n'
            #indexCalculationCode+='\tint __ipmacc_smc_index_'+v+'_'+str(rd)+'_dim2=(__ipmacc_smc_index_'+v+'_'+str(rd)+'_base%'+dim2high+')-'+self.prefix_kernel_smc_startpointer+v+'_2d;\n'
        if DEBUGSMCPRECALCINDEX:
    ...
    def constructKernel_opencl(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, template, smcinfo):
        ...
        code+='int '+self.prefix_kernel_uid_x+'=get_global_id(1);\n'
        code+='int '+self.prefix_kernel_uid_x+'=get_global_id(2);\n'
        # fetch __ipmacc_scalar into register
        for sc in args:
            if sc.find('__ipmacc_scalar')!=-1:
                vname=sc.split(' ')[-1]
...
#                if incom=='true':
    ...
    def var_copy_assignExpDetail(self, forDimOfAllKernels, blockDim):
        ...
        for i in range(0,len(self.oacc_kernelsReductions)):
            ...
            for j in range(0,len(list)):
                if GENMULTIDIMTB and len(list)>0:
                    ...
                    if root.attrib['lastlevel']=='true':
                        ...
                        if nesting==0:
                            ...
                            if GENMULTIDIMTB:
                                ...
                                #dimension_iterator=self.get_dim_iterator(str(nesting))
