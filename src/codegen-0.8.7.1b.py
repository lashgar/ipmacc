# THIS VERSION IS THE MOST RECENT VERSION BEFORE ADDING COMPRESSION SUPPORT
import sys
import re
...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def codegen_constructKernel(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, smcinfo):
        if self.target_platform=='CUDA':
            ...
            [a,b] = self.constructKernel_ispc(args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims, self.oacc_kernelsTemplates[kernelId], smcinfo)
            return [a,b]
        else:
            print 'error: unimplemented platform #' +str(getframeinfo(currentframe()).lineno)
            exit(-1)
                    ...
                    if root.attrib['independent']=='true' and root.attrib['lastlevel']=='true':
                        ...
                        if root.attrib.get('incoperator')=='+' and root.attrib.get('incstep')=='1':
                            ...
                            if root.attrib.get('incoperator')=='+' or root.attrib.get('incoperator')=='-':
                                ...
                                print 'unsupported loop increment operator> '+root.attrib.get('incoperator')
                                exit(-1)
                    elif root.attrib['independent']=='true' and nesting==0: #root.attrib['reversedepth']=='2':
