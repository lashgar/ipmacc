...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def forward_declare_attach(self):
        undecls=[]
        ...
        for i in self.active_types:
            ...
            if not self.builtin_type(tp):
                undecls.append(tp)
        if DEBUGFWDCL:
            print 'looking for declaration of: "'+'" "'.join(undecls)+'"'
        decls=srcml_get_fwdecls(codein,undecls)
        self.active_types_decl=decls
    ...
    def oacc_clauseparser_data_ispresent(self, clauses):
