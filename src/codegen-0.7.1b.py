            ...
            for [v, t, st, p, dw, up, div, a, dimlo, dimhi, w_dw, w_up, dim2low, dim2high, pivot2, dw2range, up2range, w_dw2range, w_up2range] in smcinfo:
                ...
                if dim2high=='' and dim2low=='':
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE' or st=='FETCH_CHANNEL':
                        ...
                        #   start address
                        datafetch+="\n // FINDING BEGINNING OF TILE STARTED"
                        datafetch+='\n__syncthreads();\n'
                        ...
                        datafetch+='\n__syncthreads();\n'
                        #  end address 
                        datafetch+="\n // FINDING BEGINNING OF TILE DONE\n"
    ...
    if(!((idx2-startptr2)<20)){\n
        REPLACECALL()\n
            assert((idx2-startptr2)<16);\n
                    ...
                    indexCalculationCode='// '+str(len(readIdxSet))+' unique indexes\n'
                    for rd in range(0,len(readIdxSet),1):
                        indexCalculationCode+='// ['+str(rd)+'] '+readIdxSet[rd]+'\n'
                        indexCalculationCode+='\tint __ipmacc_smc_index_'+v+'_'+str(rd)+'_base='+readIdxSet[rd]+';\n'
                    ...
                    if st=='READ_ONLY' or st=='READ_WRITE'  or st=='FETCH_CHANNEL':
                        kernelB=kernelB.replace(fcallen,'#undef '+v+'\n'+fcallen)
                        kernelB=kernelB.replace(fcall,indexCalculationCode)
                    pfreelist.append(a)
            for ids in pfreelist:
