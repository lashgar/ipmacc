...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def oacc_clauseparser_data_ispresent(self, clause):
        for it in ['pcopy', 'present_or_copy', 'pcopyout', 'present_or_copyout', 'pcopyin', 'present_or_copyin', 'present']:
            if clause==it:
                return True
        return False
    ...
    def var_findFuncParents(self,funcName):
        ...
        for id in range(0,count):
            ...
            if USEPYCPARSER:
                ...
                # srcML
                fname=srcml_get_parent_fcn(root,kn.strip())
...
#            print 'checking variable > \''+vars[i]+'\''
            ...
            if not priv:
                try :
                    ...
                    function_args.append(arg_type+arg_name+'__reduction')
                    ...
                    if not self.iskeyword(vars[i]):
                        print 'WARNING: Could not determine the type of identifier used in the kernel: '+vars[i]
                        print '\tignoring undefined variable, maybe a macro solves problem!'
