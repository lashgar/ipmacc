...
class codegen(object):
    def __init__(self, target, foname, nvcc_args, optimizations):
        ...
        self.algorithm_execution_width='512'
        #self.blockDim='256'
...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def get_preferred_blockDim(self, kernelId):
        ...
        for [algoKId, algoId, algoBody, algoClause, revdepnest] in self.oacc_algoReg:
            if algoKId==kernelId:
                ...
                if revdepnest=='0':
                    blockDimx=self.algorithm_execution_width
                    blockDimy='2'
                    blockDimz='2'
                else:
    ...
    def oacc_kernelsConfig_getDecl(self, kernelId):
        ...
        if   ndim==3:
            ...
            if blockDimz=='PREDEF':
                blockDimz=blockDimz_reconf if blockDimz_reconf!='' else self.blockDim_cuda_xyz
        elif ndim==2:
            if blockDimx=='PREDEF':
                blockDimx=blockDimx_reconf if blockDimx_reconf!='' else self.blockDim_cuda_xyz
            if blockDimy=='PREDEF':
    ...
    def appendKernelToCode_cuda(self, kerPro, kerDec, kerId, forDims, args):
        ...
        if nctadim>1 or GENMULTIDIMTB:
            ...
            kernelInvoc+=self.prefix_kernel_gen+str(kerId)+('_compression' if len(self.oacc_kernelsComp[kerId])!=0 else '')+'<<<__ipmacc_gridDim,__ipmacc_blockDim>>>('+(','.join(callArgs))+');'
            if PROFILER:
            ...
            if PROFILER:
                kernelInvoc+='acc_profiler_start();\n'
            kernelInvoc+=self.prefix_kernel_gen+str(kerId)+('_compression' if len(self.oacc_kernelsComp[kerId])!=0 else '')+'<<< griddim_x, '+blockDim+'>>>('+(','.join(callArgs))+');'
            if PROFILER:
                kernelInvoc+='acc_profiler_end(0);\n'
    ...
    def cuda_sort_get_func(self):
        ...
        for tp in self.oacc_algoSort_Types2Overload:
            ...
            declbody ="""
                ...
                """
            calls.append([fname, prototype, declbody])
            fname = 'compare2_'+tp
            prototype = 'uint '+fname+'(const '+tp+' &left, const '+tp+' &right)'
            declbody ="""
    ...
    def algo_cuda(self, algoBody, algoClause, kernelId, revdepnest):
        for [i0, i3] in clauseDecomposer_break(algoClause):
            ...
            if i0.strip()=='sort':
                code=''
                code+="""// """+i3+"""\n"""
                for variable in i3.split(','):
                    vname  = variable.split('[')[0]
                    config = variable.split('[')[1].split(']')[0]
                    range_low = config.split(':')[0]
                    range_hig = config.split(':')[1]
                    try:
                        varNameList=self.oacc_kernelsVarNams[kernelId]
                        varTypeList=self.oacc_kernelsVarTyps[kernelId]
                    ...
                    if True:
                        ...
                        decl ='__shared__ '+vtype_noptr+' '+vname+'_local '+'[('+bdy+')*('+bdz+')*('+range_hig+'-'+range_low+')];\n'
                        code+="""// fetch
                             {
                                 int __ipmacc_tmp_i=0;
                                 for(__ipmacc_tmp_i="""+range_low+"""; __ipmacc_tmp_i<("""+range_hig+"""); __ipmacc_tmp_i++){
                                 // for(__ipmacc_tmp_i=threadIdx.x+"""+range_low+"""; __ipmacc_tmp_i<("""+range_hig+"""); __ipmacc_tmp_i+=blockDim.x){ // we might use this line if the data is shared
                                     """+vname+"""_local[__ipmacc_tmp_i-"""+range_low+"""+"""+offset+"""]="""+vname+"""[__ipmacc_tmp_i];
                                 }
                             }
                             __syncthreads();
                             // sort
                             mergeSortSharedCall<"""+vtype_noptr+""",1U>(&"""+vname+"""_local["""+offset+"""], """+range_hig+'-'+range_low+""", &__accelerator_compare1_"""+vtype_noptr+""", &__accelerator_compare2_"""+vtype_noptr+""");
                             __syncthreads();
                             // writeback
                             {
                                 int __ipmacc_tmp_i=0;
                                 for(__ipmacc_tmp_i="""+range_low+"""; __ipmacc_tmp_i<("""+range_hig+"""); __ipmacc_tmp_i++){
                                 //for(__ipmacc_tmp_i=threadIdx.x+"""+range_low+"""; __ipmacc_tmp_i<("""+range_hig+"""); __ipmacc_tmp_i+=blockDim.x){ // we might use this line if the data is shared
                                     """+vname+"""[__ipmacc_tmp_i]="""+vname+"""_local[__ipmacc_tmp_i-"""+range_low+"""+"""+offset+"""];
                                 }
                             }
                             __syncthreads();
                          """
                        ...
                        code+="""
                             // sort
                             mergeSortSharedCall<"""+vtype_noptr+""",1U>("""+vname+""", """+range_hig+'-'+range_low+""", &__accelerator_compare1_"""+vtype_noptr+""", &__accelerator_compare2_"""+vtype_noptr+""");
                          """
                    sort_dev_fname = ''
                    sort_dev_prototype = ''
                    sort_dev_decl = ''
...
#            code+='int '+i+';\n'
    ...
    def kernel_forSize_list(self, root, lookingdepth, currentdepth):
        ...
        if root.tag=='pragma' and root.attrib['directive']=='algorithm':
            itercount=self.algorithm_execution_width
        ...
        try:
            if root.tag=='pragma':
                if root.attrib.get('directive')=='kernels':
                    ...
                    algoBody=''
                    self.oacc_kernelsConfig_update(id, self.algorithm_execution_width)
                    for ch in root:
