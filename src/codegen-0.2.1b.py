...
#            self.oacc_kernels_forDim.append(forDims)
            ...
            [args, declaration, implicitCopy]=self.find_kernel_undeclaredVars_and_args(kernelBody, i)
...
#            exit(-1)
    ...
    def perform_implicit_copy(self,kernelId,scopeVarsNames,scopeVarsTypes,implicitCopies):
        ...
        code_copyout=''
        if DEBUG>1:
            print 'Implict Copy Checking for implicit copy'
    ...
    def find_kernel_undeclaredVars_and_args(self, kernelBody, kernelId):
        ...
        # report stats
        if VERBOSE==1:
            print 'kernels region > kernel arguments: '+', '.join(function_args)
        ...
        try:
            if root.tag=='pragma':
                ...
                if root.attrib['independent']=='true': #independent:
                    ...
                    if nested==1:
                        # last-level
                        iteratorVal=(root.attrib.get('init')+root.attrib.get('incoperator'))+'('+self.prefix_kernel_uid+'%('+forDims[nested-1]+')'+');'
                    else:
                        # upper levels
                        iteratorVal=(root.attrib.get('init')+root.attrib.get('incoperator'))+'('+self.prefix_kernel_uid+'/('+'*'.join(forDims[0:nested-1])+')'+');'
