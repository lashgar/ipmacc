...
class codegen(object):
    ...
    def oacc_clauseparser_deviceptr(self,clause):
        ...
        #regex = re.compile(r'([A-Za-z0-9\ ]+)\((.+?)\)')
        for i in regex.findall(clause):
            if str(i[0]).strip()=='deviceptr':
                try:
                    return i[3].strip().replace(',',' ')
                except:
    ...
    def cuda_append_closecondition(self,cond):
        return '}\n'
    def cuda_append_implicitcopy(self,inout,kid):
    ...
    def code_descendentRetrieve(self, root, depth):
        ...
        if root.tag == 'pragma' and root.attrib.get('directive')=='kernels':
            ...
            self.code=self.code+self.cuda_append_implicitcopy('in',self.oacc_kernelId)
            ...
            #   - speculative implicit memory allocation/transfers
            self.code=self.code+self.cuda_append_implicitcopy('out',self.oacc_kernelId)
            #   - async
    ...
    def var_copy_genCode(self):
        ...
        for i in range(0,len(self.oacc_copys)):
            ...
            for j in self.oacc_copys[i].split('\n'):
                ...
                if incom=='true':
                    codeC=codeC+'ipmacc_prompt("IPMACC: memory copyin '+varname+'\\n");\n'
                    codeC=codeC+'cudaMemcpy('+dname+','+varname+','+size+','+'cudaMemcpyHostToDevice'+');\n'
                elif incom=='false':
                    codeC=codeC+'ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
...
#                regex = re.compile(r'([a-zA-Z0-9_]*)([=])(\")([a-zA-Z0-9_:\(\)\*\ ]*)(\")')
                ...
                if varmapper_allocated_found==False:
                    ...
                    if present=='true':
                        ...
                        codeM=codeM+'}\n'
                    self.varmapper_allocated.append((parentFunc,dname))
                # generate cudaMemcpy in/out code
                passbyref='' if reduc=='' else '&'
#                if incom=='true':
                codeCin=codeCin+'ipmacc_prompt("IPMACC: memory copyin '+varname+'\\n");\n'
                codeCin=codeCin+'cudaMemcpy('+dname+','+passbyref+varname+','+size+','+'cudaMemcpyHostToDevice'+');\n'
#                elif incom=='false':
                codeCout=codeCout+'ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                codeCout=codeCout+'cudaMemcpy('+passbyref+varname+','+dname+','+size+','+'cudaMemcpyDeviceToHost'+');\n'
            self.code_include=self.code_include+vardeclare
            self.code=self.code.replace(self.prefix_dataalloc+str(i)+'();',codeM)
...
#                list[j]+=' predty="'+tp+'"'
    ...
    def cuda_append_kernel_to_code(self, kerPro, kerDec, kerId, forDims, args):
...
#        self.astRoot = tree.getroot()
    ...
    def cuda_reduce_variable(self, var, type, op, ctasize):
    ...
    def cuda_construct_kernel(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims):
        ...
        if len(reduinfo)>0:
            ...
            for [v, i, o, a, t] in reduinfo:
                ...
                kernelB=kernelB.replace(fcall,self.cuda_reduce_variable(v,t,o,ctasize)+fcall)
                types.append(t)
                rfreelist.append(a)
            reduinfo.reverse()
    ...
    def oacc_clauseparser_loop_isindependent(self,clause):
        ...
        for it in regex.findall(clause):
            ...
            if it[0].strip()=='independent':
                ...
                indep=False
            # private vars
            elif it[0].strip()=='private' and it[3].strip()!='':
                #private=(it[3].strip().replace(',',' ')+' '+private).strip()
                private.append(it[3].strip())
            # reduction vars
            elif it[0].strip()=='reduction' and it[3].strip()!='':
                reduction.append(it[3].strip())
            # reduction vars
...
#        print 'again, kernel iterators > '+','.join(kernelLoopIterators)
        function_args=[]
        implicitCopies=[]
...
#            print 'checking variable > \''+vars[i]+'\''
            itr=True
            try:
                idx=kernelLoopIterators.index(vars[i].strip())
            except:
                itr=False
            if itr:
                continue
            ...
            if priv and redu:
                ...
                priv=False
            # 5- append function arguments, and find undefined variables (ignore non-pointer)
            undef=False
            pointr=False
            if not priv:
                try :
                    idx=scopeVarsNames.index(vars[i])
                    function_args.append(scopeVarsTypes[idx]+('* ' if redu else ' ')+scopeVarsNames[idx])
...
#            print 'it could be something like -> '+self.kernel_forSize_CReadable(self.find_kernel_forSize_Recursive(self.oacc_kernels[i]))
            if DEBUGLD or DEBUGVAR>0:
                 print 'iterators are '+','.join(iterators)
            iterators = list(set(iterators))
        ...
        for i in range(0,len(self.oacc_kernels)):
            ...
            if DEBUGPRIVRED:
                ...
                self.debug_dump_privredInfo('reduction',reduInfo)
            # loop continue
            [kernelPrototype, kernelDecl]=self.cuda_construct_kernel(args, declaration, kernelBody, i, privInfo, reduInfo, self.blockDim, forDims)
            self.cuda_append_kernel_to_code(kernelPrototype, kernelDecl, i, forDims, args)
