...
VERBOSE=0
ERRORDUMP=True
REDUCTION_TWOLEVELTREE=True # two-level tree reduction is the default policy. setting the control to False, generates only CUDA code which is supported on limited number of devices. 
USEPYCPARSER=True
...
#        str+='dimloops="'+root.attrib.get('dimloops')+'" '
    ...
    def oacc_clauseparser_data(self, clauses):
        ...
        for it in ['copy', 'pcopy', 'present_or_copy', 'copyout', 'pcopyout', 'present_or_copyout', 'copyin', 'pcopyin', 'present_or_copyin', 'create', 'present']:
    ...
    def codegen_constructKernel(self, args, decl, kernelB, kernelId, privinfo, reduinfo, ctasize, forDims):
        if self.target_platform=='CUDA':
            ...
            print 'error: unimplemented platform'
            exit(-1)
    def codegen_memAlloc(self, dvar, size, hvar, type, scalar_copy):
# OLD IMPLEMENTATION:
#       code=''
#        if self.target_platform=='CUDA':
...
# API-based IMPLEMENTATION FIXME
        ...
        if self.target_platform=='CUDA':
            code=dvar+'=('+type+')'+'acc_create((void*)'+amp+hvar+','+size+');\n'
        elif self.target_platform=='OPENCL':
            code=dvar+'=(cl_mem)'+'acc_create((void*)'+amp+hvar+','+size+');\n'
        else:
            print 'error: unimplemented platform'
            exit(-1)
        return code
    ...
    def codegen_accCopyin(self, host, dev, bytes, type, present):
        code=''
        if self.target_platform=='CUDA':
            code+=dev+'=('+type+')acc_'+present+'copyin((void*)'+host+','+bytes+');\n'
        elif self.target_platform=='OPENCL':
    ...
    def codegen_accPCopyout(self, host, dev, bytes, type):
        code=''
    ...
    def appendKernelToCode_opencl(self, kerPro, kerDec, kerId, forDims, args):
        ...
        kernelInvoc+='clBuildProgram(__ipmacc_clpgm'+kerId_str+', 0, NULL, __ipmacc_clcompileflags'+kerId_str+', NULL, NULL);\n'
    ...
    def reduceVariable_opencl(self, var, type, op, ctasize):
        ...
        if REDUCTION_TWOLEVELTREE:
            code+=var+'[get_group_id(0)]='+arrname+'[0];\n'
        else:
...
# TO HERE
    ...
    def var_findFuncParents(self,funcName):
        ...
        for id in range(0,count):
            ...
            if USEPYCPARSER:
                ...
                #srcML
                print 'Error! unimplemented AST parser!'
                exit(-1)
    ...
    def var_find_size(self, varName, funcName, root):
        ...
        if USEPYCPARSER:
            ...
            print 'Error! unimplemented AST parser!'
            exit(-1)
        print('Fatal Internal Error! expected function is not found in the code!')
        exit(-1)
            ...
            for j in cp_expression.split('\n'):
                ...
                if size.find('dynamic')!=-1:
                    if size.count('dynamic')!=len(dim) and not self.oacc_data_dynamicAllowed(clause):
                        print 'Error: data clause copy statement and the variable dimension are not matched! variable name: '+varname+' - '+clause
                        exit(-1)
                    for repa in dim:
                        if repa.find(':')==-1:
                ...
                if varmapper_allocated_found==False:
                    ...
                    if present=='true':
                        ...
                        codeM+=self.codegen_memAlloc(dname,size,varname,type,scalar_copy)
                        codeM+='}\n'
                    else:
                ...
                if clause=='copyin' or clause=='copy':
                    ...
                    codeCin+=self.codegen_accCopyin(('&' if scalar_copy else '')+varname, dname, size, type, '' )
                if clause=='pcopyin' or clause=='pcopy':
                    codeCin+='ipmacc_prompt("IPMACC: memory copyin '+varname+'\\n");\n'
                    codeCin+=self.codegen_accCopyin(('&' if scalar_copy else '')+varname, dname, size, type, 'p')
                if clause=='present':
                ...
                if clause=='copyout' or clause=='copy' or clause=='pcopyout' or clause=='pcopy':
                    codeCout+='ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                    #codeC+=self.codegen_memCpy(('&' if scalar_copy else '')+varname, dname, size, 'out')
                    #codeM+=self.codegen_memAlloc(dname,size,varname,type,scalar_copy)
                    codeCout+=self.codegen_accPCopyout(('&' if scalar_copy else '')+varname, dname, size, type)
                ...
                if varmapper_allocated_found==False:
                    ...
                    if present=='true':
                        ...
                        if REDUCTION_TWOLEVELTREE:
                            ...
                            codeM+= self.codegen_memAlloc(dname,size,arrname,type,scalar_copy)
                        else:
                            codeM+= self.codegen_memAlloc(dname,size,varname,type,scalar_copy)
...
#                if incom=='true':
                if reduc=='':
                    ...
                    codeCin+=self.codegen_accCopyin(passbyref+varname, dname, size, type, 'p')
                    codeCout+='ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                    #codeCout+=self.codegen_memCpy(passbyref+varname, dname, size, 'out')
                    codeCout+=self.codegen_accPCopyout(passbyref+varname, dname, size, type)
                else:
                    if REDUCTION_TWOLEVELTREE:
                        ...
                        codeCout+=self.codegen_accPCopyout(arrname, dname, size, type)
                        iterator =self.prefix_kernel_reduction_iterator
    ...
    def var_copy_assignExpDetail(self, forDimOfAllKernels, blockDim):
        ...
        if USEPYCPARSER:
            ...
            #srcML
            print 'Error! Unimplemented AST generator!'
            exit(-1)
...
#            code+='int '+i+';\n'
        for i in kernelLoopIteratorsPar+kernelLoopIteratorsSeq:
            ...
            try :
                ...
                defOutKernel=False
                print 'Warning: Could not determine the type of loop iterator used in the kernel: '+i
            if not defOutKernel:
                # TODO: avoid warning
                # check inside the kernel for the definition 
                # if not exist report error
                print('\tunimplemented fallback')
                #exit(-1)
