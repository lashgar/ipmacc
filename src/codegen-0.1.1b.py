...
class codegen(object):
    ...
    #
    def oacc_data_gencopy(self,clause,type,inout,persistent):
        ...
        for i in regex.findall(clause):
            if str(i[0]).strip()==type:
                for j in str(i[1]).split(','):
                    ...
                    # persistent
                    tcode=tcode+'persistent="'+persistent+'" '
                    # dimensions
                    regex2 = re.compile(r'\[(.+?)\]')
    ...
    def code_descendentRetrieve(self,root,depth):
        ...
        if root.tag == 'pragma' and root.attrib.get('directive')=='kernels':
            ...
            if root.tag == 'for':
                ...
                # dump pragma copyin before region
                expression=''
                for it in ['copyin', 'copy', 'pcopy', 'pcopyin']:
                    expression=expression+self.oacc_data_gencopy(str(root.attrib.get('clause')),it,'in','true')
                self.code=self.code+('\t'*depth)+self.prefix_datacp+str(self.oacc_copyId)+'();'
                self.oacc_copys.append(expression)
                #self.oacc_copyWb(self.oacc_copyId,expression)
                self.oacc_copyId=self.oacc_copyId+1

            for child in root:
                self.code_descendentRetrieve(child,depth+1)
            # dump pragma copy out after the region
            if (root.tag=='pragma' and root.attrib.get('directive')=='data'):
                # dump pragma copyin before region
                expression=''
                for it in ['copyout', 'copy', 'pcopy', 'pcopyout']:
                    expression=expression+self.oacc_data_gencopy(str(root.attrib.get('clause')),it,'out','true')
                self.code=self.code+('\t'*depth)+self.prefix_datacp+str(self.oacc_copyId)+'();'
                self.oacc_copys.append(expression)
                #self.oacc_copyWb(self.oacc_copyId,expression)
                self.oacc_copyId=self.oacc_copyId+1
    ...
    def declareRecursive(self,astNode):
        ...
        if astNode.tag=='PtrDecl':
            ...
            code='(unkown)*'+self.declareRecursive(astNode[0])
    ...
    def var_find_size(self, varName, funcName):
                ...
                for var in funcBody.findall(".//Decl"):
                    ...
                    if var.get('uid').strip()==varName.strip():
                        ...
                        if size.find('dynamic')!=-1:
                            print('Error: dynamic allocation detected ('+varName.strip()+')')
                            exit(-1)
        ...
        for i in range(0,len(self.oacc_copys)):
            ...
            for j in self.oacc_copys[i].split('\n'):
                ...
                if varmapper_allocated_found==0:
                    ...
                    vardeclare=vardeclare+'short '+dname+self.suffix_persistent+'='+('0')+';\n' # for now we assume are variables are persistent
                    # generate cudaMemalloc
                    if persistent=='true':
                        code=code+'if(!'+dname+self.suffix_persistent+'){\n'
                        code=code+dname+self.suffix_persistent+'++;\n'
                        code=code+'if (getenv("IPMACC_VERBOSE")) printf("IPMACC: memory allocation '+dname+'\\n");\n'
                        code=code+'cudaMalloc((void**)&'+dname+','+size+');\n'
                        code=code+'}\n'
                    else:
                        code=code+'if(!'+dname+self.suffix_persistent+'){\n'
                        code=code+'if (getenv("IPMACC_VERBOSE")) printf("IPMACC: memory allocation '+dname+'\\n");\n'
                ...
                if incom=='true':
                    code=code+'if (getenv("IPMACC_VERBOSE")) printf("IPMACC: memory copyin '+varname+'\\n");\n'
                    code=code+'cudaMemcpy('+dname+','+varname+','+size+','+'cudaMemcpyHostToDevice'+');\n'
                else:
                    code=code+'if (getenv("IPMACC_VERBOSE")) printf("IPMACC: memory copyout '+varname+'\\n");\n'
                    code=code+'cudaMemcpy('+varname+','+dname+','+size+','+'cudaMemcpyDeviceToHost'+');\n'
            self.code_include=self.code_include+vardeclare
            self.code=self.code.replace(self.prefix_datacp+str(i)+'();',code)
            ...
            for j in range(0,len(list)-1):
                ...
                for (a, b, c, d, e) in regex.findall(list[j]):
                    if a=='varname':
                        ...
                        list[j]=list[j]+' size="'+self.var_find_size(d,self.oacc_copysParent[i])+'"'
    ...
    def append_kernel_to_code(self, kerPro, kerDec, kerId, forDims, args):
        ...
        callArgs=[]
        for i in args:
            callArgs.append(self.varmapper_getDeviceName(self.oacc_kernelsParent[kerId], i.split(' ')[1]))
    ...
    def clauseParser_loop_isindependent(self,clause):
        k=clause.replace('independent','independent()')
        regex=re.compile(r'([A-Za-z0-9\ ]+)([\(])([A-Za-z0-9\ ]*)([\)])')
        for it in regex.findall(k):
            #print ''.join(it).strip()
    ...
    def find_kernel_undeclaredVars_and_args(self, kernelBody, kernelId):
        ...
        for var in root.findall(".//Decl"):
            try :
                ...
                # it was function call, ignore it
                k=True
        print 'Undelared Variables: '+','.join(vars)
        ...
        for i in range(0,len(vars)):
            ...
            try :
                ...
                print 'Could not determine the type of variable used in the kernel: '+vars[i]
                print 'ignoring undefined variable, maybe a macro solves proble!'
                #exit (-1)
        print 'Function Arguments: '+','.join(function_args)
        ...
        try:
            if root.tag=='pragma':
                if root.attrib.get('directive')=='kernels':
                    ...
                    indep=self.clauseParser_loop_isindependent(root.attrib.get('clause'))
                    for ch in root:
                        #print str(indep)
