...
#                if incom=='true':
                ...
                codeCin=codeCin+'cudaMemcpy('+dname+','+varname+','+size+','+'cudaMemcpyHostToDevice'+');\n'
#                elif incom=='false':
                codeCout=codeCout+'ipmacc_prompt("IPMACC: memory copyout '+varname+'\\n");\n'
                codeCout=codeCout+'cudaMemcpy('+varname+','+dname+','+size+','+'cudaMemcpyDeviceToHost'+');\n'
            self.code_include=self.code_include+vardeclare
            self.code=self.code.replace(self.prefix_dataalloc+str(i)+'();',codeM)
...
#        self.astRoot = tree.getroot()
    ...
    def find_kernel_undeclaredVars_and_args(self, kernelBody, kernelId):
        ...
        # fourth, find the function arguments
...
#            print 'checking variable > \''+vars[i]+'\''
            ...
            if len(self.oacc_kernelsManualPtrs):
                for mv in self.oacc_kernelsManualPtrs[kernelId].split(','):
                    if mv.strip()==vars[i].strip():
                        manualvar=True
                        break
            # 4- append function arguments, and find undefined variables (ignore non-pointer)
            undef=False
            pointr=False
            try :
        ...
        return [function_args, code, implicitCopies]
    ...
    def var_kernel_genPlainCode(self, id, root, indent, independent):
        code=''
        try:
            if root.tag=='pragma':
                if root.attrib.get('directive')=='kernels':
                    code=code+'{\n'
                    for ch in root:
                        code=code+self.var_kernel_genPlainCode(id, ch, indent+1, False)
                    code=code+'}\n'
                elif root.attrib.get('directive')=='loop':
                    indep=self.oacc_clauseparser_loop_isindependent(root.attrib.get('clause'))
                    for ch in root:
                        #print str(indep)
                        code=code+self.var_kernel_genPlainCode(id, ch, indent+1, indep)
                else:
                    print('Invalid: '+root.attrib.get('directive')+' directive in region!')
                    exit(-1)
            elif root.tag=='for':
...
#            print 'it could be something like -> '+self.kernel_forSize_CReadable(self.find_kernel_forSize_Recursive(self.oacc_kernels[i]))
            ...
            kernelBody=self.var_kernel_genPlainCode(i, self.oacc_kernels[i], 0, False)
            [args, declaration, implicitCopies]=self.find_kernel_undeclaredVars_and_args(kernelBody, i)
