...
class codegen(object):
    ...
    #
    def oacc_dataclause_parser(self,clause,type,inout,present):
    ...
    def oacc_dataclauses_parser(self, clauses):
        expressionIn=''
        expressionAlloc=''
        expressionOut=''
        copyoutId=-1
        ...
        for it in ['copyin', 'copy']:
            expressionIn=expressionIn+self.oacc_dataclause_parser(clauses,it,'true','false')
        for it in ['pcopy', 'present_or_copy', 'pcopyin', 'present_or_copyin']:
            expressionIn=expressionIn+self.oacc_dataclause_parser(clauses,it,'true','true')
        # allocate-only
        for it in ['create']:
            expressionAlloc=expressionAlloc+self.oacc_dataclause_parser(clauses,it,'create','true')
        # copy-out
        for it in ['copyout', 'copy']:
            expressionOut=expressionOut+self.oacc_dataclause_parser(clauses,it,'false','false')
        for it in ['pcopy', 'present_or_copy', 'pcopyout', 'present_or_copyout']:
            expressionOut=expressionOut+self.oacc_dataclause_parser(clauses,it,'false','true')
    ...
    def code_descendentRetrieve(self, root, depth):
        ...
        if root.tag == 'pragma' and root.attrib.get('directive')=='kernels':
            ...
            expressionOut=''
            copyoutId=-1
            [expressionIn, expressionAlloc, expressionOut, copyoutId] = self.oacc_dataclauses_parser(str(root.attrib.get('clause')))
            if DEBUG==1:
