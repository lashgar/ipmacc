...
class Node(object):
    ...
    def showXml(self, buf=sys.stdout, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None):
        """ Pretty print the Node and all its attributes and
            children (recursively) to a buffer.
...
#        else:
        ...
        if self.attr_names:
            ...
            for v in vlist:
                if str(v)!='[]':
                    t2append=str(v).replace('[','').replace(']','').replace(',','').replace('\'','').replace('&','&amp;').replace('>','&gt;').replace('<','&lt;')
                    if t2append!='const' and t2append!='static':
                        attrstr.append(t2append)
#            attrstr = ', '.join('%s' % v for v in vlist)
#            buf.write(attrstr)

        if ','.join(attrstr)!='':
            buf.write(' uid=\''+','.join(attrstr)+'\' >\n')
        else:
            buf.write('>\n')
